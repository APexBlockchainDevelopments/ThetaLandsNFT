...
contract ThetaLandsNFT is TNT721 {

    address public _admin; //me
    ...
    constructor (string memory name, string memory symbol, string memory link) public TNT721(name, symbol) { 
        _admin = msg.sender;
        uri = link;
     ...
     function mintTo() public payable{
        if(msg.sender != _admin){
            require(msg.value >= 200000000000000000000, "Payment not enough");
        }
        uint mintIndex = totalSupply();
        _safeMint(msg.sender, mintIndex);
        _setTokenURI(mintIndex, uri);
    ...
    function newURI(string memory newUri, uint256 i) public{
        require(msg.sender == _admin, "You are not allowed to do that");
        //1 Need to change variable
        uri = newUri;
        _setTokenURI(i, uri);
